...
def KFAC_plotter():
    ...
    parser.add_argument("--data1", type=str, default="math_sycophantic")
    parser.add_argument("--data2", type=str, default="math_truthful")
    parser.add_argument("--num_batches", type=int, default=25)
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--cache_dir", type=str, default="./stats_cache_math")
    ...
    plt.savefig("similarity_plot_math.png")
    plt.close()
    ...
    plt.plot(layers, gate_data[:, 1] , label=f"gate({args.data1})", color='blue')
    plt.plot(layers, up_data[:, 1], label=f"up({args.data1})", color='red')
    plt.plot(layers, down_data[:, 1], label=f"down({args.data1})", color='green')
    plt.plot(layers, gate_data[:, 2], label=f"gate({args.data2})", color='blue', linestyle="--")
    plt.plot(layers, up_data[:, 2], label=f"up({args.data2})", color='red', linestyle="--")
    plt.plot(layers, down_data[:, 2], label=f"down({args.data2})", color='green', linestyle="--")
    plt.xlabel("Layer Index")
    plt.ylabel("Trace Magnitude")
    plt.legend()
    #plt.grid(False, alpha=0.3)
    plt.yscale('log')
    plt.savefig("magnitude_plot_anthropic.png")
    plt.close()
...
def weight_plotter():
    ...
    parser.add_argument("--cache_dir", type=str, default="./stats_cache_math")
    parser.add_argument("--seed", type=int, default=2025)
    parser.add_argument("--weight_proj", type=str, default="gate")
    ...
    fig_def.suptitle(f"math syc vs math truthful task detection via K-FAC ({args.weight_proj})")
    fig_def.tight_layout()
    fig_def.savefig("graphs/all_layers_kfac_2_math.png", dpi=300)
    plt.close(fig_def)

    fig_comp.suptitle(f"Math syc task detection K-FAC vs Activation Projection ({args.weight_proj})")
    fig_comp.tight_layout()
    fig_comp.savefig("graphs/all_layers_act_2_math.png", dpi=300)
    plt.close(fig_comp)

    fig_all.suptitle(f"Math syc task detection K-FAC vs Grad Projection  ({args.weight_proj})")
    fig_all.tight_layout()
    fig_all.savefig("graphs/all_layers_combined_2_math.png", dpi=300)
    plt.close(fig_all)

    print("Done. Graphs saved to ./graphs/math")
